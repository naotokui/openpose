...
DEFINE_string(write_zmq_json,      "",             "Directory to write people pose data being sent on ZMQ");
...
public:
    UserOutputClass(){
      ...
      void *_publisher = zmq_socket (context, ZMQ_PUB);
      int rc = zmq_bind (_publisher, "tcp://*:5555");
      if (rc == 0){
        zmq_publisher = _publisher;
      } else {
        op::error("Failed to open ZMQ publisher socket");
      }
        ...
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            ...
            for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
            {
                ...
                for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++)
                {
                    ...
                    {
                        fs << poseKeypoints[{person, bodyPart, xyscore}];
                        if (xyscore < 2) fs << ", ";
                    }
                    fs << "]";
                    if (bodyPart < poseKeypoints.getSize(1) - 1) fs << ",";
                }
                fs << "]\n}\n";
                if (person < poseKeypoints.getSize(0) - 1) fs << ",";
            }
            fs << "]\n}";
            // op::log(fs.str());
            ...
            int size = fs.tellg();
            zmq_send (zmq_publisher, fs.str().c_str(), size, 0);

            // Export JSONs
            if (!FLAGS_write_zmq_json.empty()) {
                char fname[256];
                boost::filesystem::path p(FLAGS_video);
                std::string rawname = p.stem().string();

                sprintf(fname, "%s/%s_%05ld.json", FLAGS_write_zmq_json.c_str(), rawname.c_str(), count++);
                std::ofstream fs_json(fname);
                fs_json << fs.str();
            }
        }
        else
